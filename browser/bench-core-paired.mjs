...
const NODES = {
  ...
  case3: [10, 50, 100, 200, 500, 1000, 2000],
  case4: [10, 50, 100, 200, 500, 1000, 2000],
};
...
function c1Baseline(makeDoc, n) { // A2 optimized: meta + Map index
  ...
  return withDispose(idem(() => { let ok = 0; for (const { id } of inputs) { const node = doc.getElementById(id); const meta = metaIndex.get(id); const t = meta ? meta.getAttribute("content") : null; if (String(node.getAttribute("value") ?? "") === String(t ?? "")) ok++; } return ok; }), doc);
}
function c1Proposed(makeDoc, n) { // B registry
  ...
  return withDispose(idem(() => { let ok = 0; for (const node of nodes) { const eid = weakNodeMap.get(node); if (!eid) continue; const e = indexMap.get(eid); if (e && String(node.getAttribute("value") ?? "") === String(e.truth ?? "")) ok++; } return ok; }), doc);
}
...
function c3Baseline(makeDoc, n) { const doc = makeDoc(); const root = c3Build(doc, n); const base = snapshot(root); return withDispose(idem(() => { snapDiff(base, snapshot(root)); }), doc); }
...
// Attach dispose() so runPaired can release a prepare fn's document(s) after
// measurement. closeDoc is a no-op in the browser, effective in JSDOM.
